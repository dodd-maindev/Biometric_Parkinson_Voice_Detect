...
from src.infrastructure.extractors.gammatone_cepstral_extractor import GammatoneCepstralExtractor
...
class BaselineReplicationPipeline:
    ...
    def __init__(
        self, segmentation_cache_directory: Path,
        validation_strategy: Optional[IValidationStrategy] = None,
        clear_segment_cache: bool = False, output_model_path: Optional[Path] = None,
    ) -> None:
        ...
        self._feat_cache = self._cache_dir.parent / "features_acoustic_gtcc.npz"
        ...
        if clear_segment_cache:
            if self._cache_dir.exists():
                shutil.rmtree(self._cache_dir)
            if self._feat_cache.exists():
                self._feat_cache.unlink()
            print(f"  Cleared segment & feature cache")
    ...
    def _extract_features(self, samples: List[AudioSample]):
        ...
        features, labels, _ = FeatureExtractionService(extractors).extract_dataset(
            samples, cache_file=self._feat_cache,
        )
