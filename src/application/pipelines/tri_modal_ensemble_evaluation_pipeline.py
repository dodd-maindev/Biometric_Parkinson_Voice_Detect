...
class TriModalEnsembleEvaluationPipeline:
    ...
    def run(self, raw_samples: List[AudioSample]) -> EvaluationMetrics:
        ...
        clf_b, p_base = self._get_base_probs(feat_base, labels, samples, train_idx, test_idx)
        clf_w, p_wavlm = self._fit_ssl(feat_wavlm, labels, samples, train_idx, test_idx)
        clf_v, p_w2v2 = self._fit_ssl(feat_w2v2, labels, samples, train_idx, test_idx)
        ...
        self._save_checkpoint(clf_b, clf_w, clf_v)
        return MetricCalculationService.calculate(test_labels, fused_preds, fused_probs)

    def _save_checkpoint(self, clf_b, clf_w, clf_v) -> None:
        """Serialize tri-modal ensemble constituent models to disk."""
        if not self._output_model_path: return
        import joblib
        target = Path(self._output_model_path)
        target.parent.mkdir(parents=True, exist_ok=True)
        joblib.dump({"baseline": clf_b, "wavlm": clf_w, "w2v2": clf_v, "weights": self._weights, "threshold": self._threshold}, target)
        print(f"  Saved tri-modal ensemble checkpoint to: {target}")
    ...
    def _fit_ssl(self, feat, labels, samples, train_idx, test_idx, grid=None):
        """Fit a tuned SVM on training split and return model with test probabilities."""
    ...
    def _log_threshold_diagnostics(self, y_true: np.ndarray, y_prob: np.ndarray) -> None:
        ...
        for th in [0.48, 0.50, 0.52, 0.54, 0.56]:
            m = MetricCalculationService.calculate(y_true, (y_prob >= th).astype(np.int32), y_prob)
