...
class SupportVectorClassifier(IClassifierModel):
    ...
    def _fit_with_grid_search(
        ...
        """Grid search with subject-aware stratified CV to avoid leakage."""
        cv_strategy = self._build_cv_strategy(groups)
        grid = GridSearchCV(
            SVC(
    ...
    def _build_cv_strategy(self, groups):
        """Select CV strategy: subject-grouped if groups available."""
        ...
        return StratifiedGroupKFold(n_splits=10)
